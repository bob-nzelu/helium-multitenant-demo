...
class HeartBeatClient(BaseClient):
    ...
    async def fetch_blob(
        ...
        HeartBeat owns blob storage; Relay's artifact-fetch route is a thin
        authenticated proxy to HB blob bytes for HARD artifacts (signed/fixed/
        original/backend PDF, QR blob, signature). Relay calls this with its own
        HB *service* credentials (HMAC s2s) — the original ERP/user only holds
        the Relay ingest key, not HB-service creds (mirrors the
        ``downstream_auth_header`` model in ``deps._verify_hmac``).

        NEEDS-HB: HB must expose a **POST-body** blob-fetch-by-ref endpoint
        (``artifact_ref`` is a bearer capability — must NOT travel in a URL, per
        HB's own M-1 "content identifier must not appear in URLs/logs"
        discipline already applied to dedup/config). Modelled here as
        ``POST /api/blobs/fetch {blob_ref}`` returning the raw bytes with a
        ``Content-Type`` header. Shape is provisional until HB confirms.
        ...
        Returns:
            ...
            404/empty miss (the route maps that to ARTIFACT_NOT_FOUND). The
            content_type is the HB-reported MIME of the stored blob.
