...
# ── VAT pass-through (production Relay does NO VAT math) ──────────────────────
# Bob ratification 2026-06-19: VAT compute is Core/Transforma territory.


class TestBatchVATPassthrough:
    @pytest.mark.asyncio
    async def test_vat_absent_stays_none_not_computed(self, batch_service, tenant):
        # No vat_amount supplied → Relay does NOT auto-compute; stays None.
        result = await batch_service.process_batch(
            records=[_record(fee=1000.0)],  # no vat_amount
            batch_id="BVAT", tenant=tenant, trace_id="trc-vat",
        )
        assert result.processed[0].vat_amount is None

    @pytest.mark.asyncio
    async def test_caller_vat_echoed_verbatim(self, batch_service, tenant):
        # Caller-supplied vat is forwarded verbatim (Decimal), not re-rounded.
        result = await batch_service.process_batch(
            records=[_record(fee="1000.00", vat="50.005")],
            batch_id="BVAT2", tenant=tenant, trace_id="trc-vat2",
        )
        entry = result.processed[0]
        assert entry.vat_amount == Decimal("50.005")  # verbatim, no compute/round
        assert isinstance(entry.vat_amount, Decimal)

    @pytest.mark.asyncio
    async def test_no_vat_source_field(self, batch_service, tenant):
        # vat_source was removed with the auto-compute.
        result = await batch_service.process_batch(
            records=[_record(fee="1000.00", vat="75.00")],
            batch_id="BVAT3", tenant=tenant, trace_id="trc-vat3",
        )
        assert not hasattr(result.processed[0], "vat_source")

    @pytest.mark.asyncio
    async def test_fee_amount_is_decimal_echoed(self, batch_service, tenant):
        # L31 precision for money-in-transit stays: fee is Decimal, echoed.
        result = await batch_service.process_batch(
            records=[_record(fee="2500.00")],
            batch_id="BFEE", tenant=tenant, trace_id="trc-fee",
        )
        entry = result.processed[0]
        assert isinstance(entry.fee_amount, Decimal)
        assert entry.fee_amount == Decimal("2500.00")

    @pytest.mark.asyncio
    async def test_serialization_vat_null_when_absent_string_when_supplied(
        self, batch_service, tenant
    ):
        # Absent vat → JSON null; supplied → JSON string (no float on the wire).
        absent = await batch_service.process_batch(
            records=[_record(fee="1000.00")],
            batch_id="BSER1", tenant=tenant, trace_id="trc-ser1",
        )
        p_absent = json.loads(absent.to_response().model_dump_json())["processed"][0]
        assert p_absent["fee_amount"] == "1000.00"
        assert p_absent["vat_amount"] is None
        assert "vat_source" not in p_absent

        supplied = await batch_service.process_batch(
            records=[_record(fee="1000.00", vat="75.00")],
            batch_id="BSER2", tenant=tenant, trace_id="trc-ser2",
        )
        p_supplied = json.loads(supplied.to_response().model_dump_json())["processed"][0]
        assert p_supplied["vat_amount"] == "75.00"
        assert isinstance(p_supplied["vat_amount"], str)
