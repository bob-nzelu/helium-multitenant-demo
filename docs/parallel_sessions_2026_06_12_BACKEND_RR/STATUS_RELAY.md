...
- 2026-06-19 **SPRINT tick ~20 — 3 asks from Bob, routed for ARCH harmonization + HB forward.**
  1. **HARMONIZE (NEEDS FROM ARCH + HEARTBEAT) — the STANDARD internal-service webhook.** Bob clarified #22's webhook is NOT Relay-specific: Helium has **one standard internal-service webhook** — HeartBeat pushes a "what changed" signal to every service (Relay/Core/Edge/...); **one delivery type, several message types**, **HMAC**-signed; the service then **fetches** the new value. Bob: *"Is this not documented by HeartBeat?"* -> **ARCH please harmonize:** confirm/locate HB's canonical spec for (a) the webhook **protocol** + (b) the **message-type catalogue** (the change-events), minimally per service; if undocumented, **HB authors it**. Relay's L5 receiver (`webhook_auth.py`, #22) already implements the HMAC envelope (L5 = symmetric 2-header HMAC, canonical) — the platform gap is the agreed **message-type set**. Relay's expected events: config-changed, key-rotated, IQC/pack-version-bumped (L27/L32 -> HB signals Relay too), version-axis revision (policy/license/usage/auth-policy/user-perms). **This reframes #22** as "Relay implements the standard webhook + its message types," not a one-off — I conform #22's receiver to the catalogue once ARCH/HB confirm it. Relay-side view + this ask added to **#19 §15** (`6347088`).
  2. **NEEDS FROM HEARTBEAT (explicit, for ARCH to forward to the HB team) — PR #29 is BLOCKED on HB JWKS.** Relay's external OAuth ingestion (#29, Q37) cannot go live until **HeartBeat ships O1-O4: `POST /api/auth/oauth/token` + `GET /.well-known/jwks.json`** (EdDSA, `aud=helium.relay-ingest`). Relay's local-JWKS validator (Gap #2) is built, null-safe, alg-guard closed; it stays dormant (no external tokens exist) until HB's JWKS is live + `RELAY_JWKS_URL` set. **ARCH: forward to HB as the explicit dependency gating #29 go-live.** (HB OAuth O1/primitives #153/#154 in flight = same workstream; flagging the JWKS endpoint as the specific gate.)
...
- 2026-06-19 (eve) **RELAY RESUME — BOB RATIFICATION PASS actioned (4 items).** Read STATUS_ARCH BOB RATIFICATION PASS + ticks 20/27 + ledger. Executed:

  1. **🔴 VAT removal — DONE + REPORTED (per directive "report what you removed").** Ruling: production Relay does NO VAT math (Core/Transforma own it). **AUDIT** of all `services/relay/src`: `batch_external.py` was the **SOLE** production VAT-compute site (single-doc `external.py` has none; `core/tenant.py:21` is a field-name *mapping*, not math — kept; other grep hits were "priVATe"-key false-positives). **REMOVED** from `batch_external.py`: the 7.5% auto-compute `(fee*Decimal("0.075")).quantize(...,ROUND_HALF_UP)` branch, the `vat_source` label, the now-unused `ROUND_HALF_UP` import; `models.py` `BatchProcessedEntry.vat_amount` → `Optional[Decimal]` (default None), dropped `vat_source`. **KEPT** (L31 precision-in-transit): Decimal typing + JSON-string serialization + caller-`vat_amount` verbatim pass-through (null when absent). Tests rewritten (`TestBatchVATPassthrough`): absent→None (not computed), caller→verbatim, no `vat_source`. **Pushed `de858ea` on `feat/relay-external-ingestion-q37` (#29).** Green: batch_external 26 + ingest_batch 12 + amqp 20 (full suite hung on the session SSL-flake on a network test, unrelated to this change).
  2. **🔴 #22 → Ed25519 REWORK — DONE (built, pushed).** L5 reversed to Ed25519. New branch **`feat/relay-webhook-ed25519` (`921bb34`)** off `origin/main`: `WebhookVerifier` verifies HB's Ed25519 webhook sig against HB's published pubkey, **reusing the JWKSCache infra** (no second fetcher); **zero symmetric-HMAC path**; new `POST /api/webhook` route; `WebhookSignatureError` (401). 45 tests pass (20 webhook + 25 oauth) + 25 regression. The old symmetric `webhook_auth.py` on this S4 channel branch is now **superseded** (close #22's symmetric claim).
     - **⚠️ SEQUENCING for ARCH:** the L5 branch reuses `jwks_cache.py`/`oauth_validator.py`, which live on the **unmerged #29 (q37)** branch, not `main`. The agent **vendored** them onto the L5 branch so it builds/tests. **De-dup at merge:** if #29 lands first → L5 rebases on main + drops the vendored copies; if L5 lands first → it brings the infra to main + #29 de-dups. ARCH please sequence.
     - **NEEDS-FROM-HB (4, provisional contract built to):** exact signing input; exact header names (`X-HeartBeat-*`); where HB publishes the webhook pubkey (same JWKS distinct-kid vs dedicated endpoint); + the message-type catalogue (separate harmonization).
